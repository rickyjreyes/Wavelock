...
> | D | one-time enforcement is advisory file state | **FIXED at the ledger/consensus layer** — a durable `PersistentOTSReplayLedger` (`wavelock/crypto/ots_ledger.py`) is wired into block acceptance (`server.try_accept_block` → `_verify_ots_block`) and rejects any reused `one_time_key_id`/leaf, so a copied key cannot get a second OTS block accepted on a node. The host-local registry is now **defense-in-depth only**. Fully closed only when every node runs this rejection against agreed chain state. |
> | G | OTS not wired into consensus | **FIXED (single-node path)** — block acceptance routes OTS-required blocks to fail-closed OTS verification + durable replay rejection and **never** accepts legacy SIGv2 where OTS is expected. Remaining: cross-node consensus replication of the ledger and many-signature (Merkle) support. |
>
