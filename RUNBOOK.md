...
Start the node after local mining completes. Before mining another block with
the CLI, stop this node; restart it afterward so it loads the updated tip.
Use one block writer per data directory.
