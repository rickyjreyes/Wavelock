...
difference), with intermediate Phi_P^T re-permutations between squeeze rounds
(spec §4). No cryptographic primitive is used.
"""
...
def squeeze(state: PDEState, output_bits: int = spec.DEFAULT_OUTPUT_BITS) -> bytes:
    ...
    while len(bits) < output_bits:
        ...
        if len(bits) < output_bits:
            state = permute(state)   # re-permute only when more bits are needed
            cells = state.cells
