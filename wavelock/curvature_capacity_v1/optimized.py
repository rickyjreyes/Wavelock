"""NumPy implementation of CC-Core-v1 / Candidate B.

Independent of ``reference.py`` (shares only ``spec``); parity tests require
byte-for-byte agreement, including the wave round matching frozen Design A.
The only difference from Candidate A's optimized core is the injection j_B.
...
def _injection(u: np.ndarray, v: np.ndarray) -> np.ndarray:
    """j_B(u, v) = u + GAMMA*u*v = u*(1 + GAMMA*v) mod p."""
    p = _P
    return (u + spec.GAMMA * ((u * v) % p)) % p
...
def _accumulator_step(C: np.ndarray, psi_t: np.ndarray, psi_next: np.ndarray,
    ...
    j = (u + spec.GAMMA * ((u * v) % p)) % p          # j_B (linear injection)
